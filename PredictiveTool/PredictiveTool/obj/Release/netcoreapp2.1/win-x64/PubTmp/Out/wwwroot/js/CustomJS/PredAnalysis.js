    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if ($("#chkdataID").is(":checked")) {
                if ($("#hdnYearly").val() == "") { $('#tblData').empty(); draw_a_table_from_json(result.table, result.table3, Object.keys(result.table3[0]), Object.keys(result.table[0]), '#tblData', result.table5[0].per); $("#divdataID").show(); }
            }
            ...
            if ($("#chkgpID").is(":checked")) {
                ...
                if (result.table.length > 1) { Pretype = 'ShortDash'; type = 'spline'; colorcode = ['#359862', '#fa7c28', '#8db5de']; Precolorcode = ['#359862', '#fa7c28', '#8db5de']; }
                ...
                for (var a1 = 0; a1 <= result.table3.length - 1; a1++) {
                    ...
                    incser = {
                        name: Object.keys(result.table4[0])[a1 + 1],
                        data: col1,
                        //type: Pretype,
                        dashStyle: Pretype,
...
'</w:WordDocument></xml></head><body>';
    ...
    //        document.querySelector('#pngcontainer').innerHTML = '';
    //        document.querySelector('#pngcontainer').innerHTML = '<div><img src="' + imagedata + '" /></div><div style="margin-top:2pt"><span style="font-size:25pt" >' + $("#hdnsecname").val() + '</span></div><div> <img width="600px" src="' + imagedata1 + '"></img></div><div id="divtblID"></div>';
    //        document.querySelector('#divtblID').innerHTML = htmlval;

    //        var html = preHtml + document.getElementById('pngcontainer').innerHTML + postHtml;
